  ...
  return (
    ...
    >
      ...
      <div className="absolute right-[10px] top-[50%] flex -translate-y-1/2 cursor-pointer items-center gap-[15px]">
        {value && isOpen && <Search.CancelButton initKeyword={initKeyword} />}
        <Search.OpenButton isOpen={isOpen} />
      </div>
    </div>
  );
};

Search.CancelButton = ({ initKeyword }: CancelButtonProps) => {
  return (
    <div
...
Search.Input = ({ isOpen, value, initKeyword, ...props }: InputProps) => {
  return (
    <div
      onClick={(e) => (isOpen ? e.stopPropagation() : null)}
      className={cn(
        'flex h-[42px] flex-col items-start justify-center gap-[10px] rounded-[28px] border border-var-black3 bg-[#252530] p-[16px_20px] text-var-gray2 outline-none transition-all duration-300 group-hover:bg-[#17171C] group-hover:border-gradient-custom',
        isOpen ? baseStyle.openedStyle.inner : baseStyle.closedStyle.inner,
      )}
    >
      <input
        value={value}
        ...
        className={cn(
          'bg-[#17171c] placeholder-var-gray1 outline-none',
          isOpen ? baseStyle.openedStyle.input : baseStyle.closedStyle.input,
        )}
        {...props}
      />
    </div>
...
Search.OpenButton = ({ isOpen }: OpenButtonProps) => {
  return isOpen ? (
    <button type="button" className="animate-bounceRight hover:translate-x-2">
      <Image
        src="/arrow.svg"
        alt="인풋 닫기 버튼"
        width={15}
